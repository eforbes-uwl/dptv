...
impl Trace {
    pub fn add_inst(&mut self, inst: Inst, seq_num: usize) {
        if inst.stages.len() > 0 {
            // Fill the trace with dummy instructions if the instruction vector is not large enough
            while seq_num >= self.insts.len() {
                self.insts.push(Inst::default());
            }
            self.insts[seq_num] = inst;
        }
    }
}
